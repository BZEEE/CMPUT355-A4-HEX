...
class HexBoard:
    ...
    def play(self, row, col):
        ...
        # if swapping is allowed and it's player 2's first move, they can take player 1's first move
            ...
            self.swap_rule
            and len(self.move_list) == 1
            and ((row, col) == SWAP_MOVE or (row, col) == self.move_list[0])
        ):
    ...
    def is_connected(self, player, debug=False):
        ...
        if debug:
            dist_grid = [["-"] * self.size for _ in range(self.size)]
        ...
        while searchq:
            ...
            for dy, dx in ADJACENT:
                ...
                if board_val == player and (next_row, next_col) not in parent:
                    ...
                    heappush(
                        searchq,
                        (next_col if player == 1 else next_row, next_row, next_col),
                    )
    ...
    def pretty_print(self, chars=None):
        ...
        for i, row in enumerate(self.board):
            string += ("\n" + "{:>" + str(2 + i * (spacing + 1) // 2) + "} ■").format(
                str(i + 1) + ":"
            )
            for j, num in enumerate(row):
                if (
                    self._winning_group
                    and (i, j) in self._winning_group
                    and (i, j - 1) in self._winning_group
                ):
                    string += "]" + " " * (spacing - 2) + "["
                    ...
                    string += "]" + " " * (spacing - 1)
                elif self.move_list and (
                    self.move_list[-1] == (i, j)
                    or (
                    ...
                    self.move_list[-1] == (i, j - 1)
                    or (
                        self.move_list[-1] == SWAP_MOVE
                        and self.move_list[-2] == (i, j - 1)
                    )
                ):
                    string += ")" + " " * (spacing - 1)
                else:
