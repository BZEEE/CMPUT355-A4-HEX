...
def text_get_rules(default=0):
    ...
    player = [None, TextPlayer(1), build_alpha_beta_player(-1, size)]
...
def build_alpha_beta_player(player_num, size):
    ...
    return AlphaBetaPlayer(
        player_num, heuristic, search_depth, max_time, sorter, killer_moves
    )
...
def text_game(board, player):
    ...
    while board.winner == 0:
        ...
        # TODO: write the updated board values to the js script
        debug_heuristic.get_child_values(board, True)

        print("Player", board.turn % 3, "to move", "●" if board.turn > 0 else "○")

        start = default_timer()
        # TODO: encode a move from the UI and decode when getting the opponent move back to send back to UI to update
        opponentMove = player[board.turn].move(board)
        if board.turn > 0:
            print(
                "The AI made the move: {}".format(
                    str(opponentMove[0] + 1) + ", " + str(opponentMove[1] + 1)
                )
            )
...
if __name__ == "__main__":
    ...
    #     try:
    #         preset = int(
    #             input("Preset number 1-%d? (0 for custom):" % (len(DEFAULTS) - 1))
    #         )
    #     except ValueError:
    #         pass
    main(use_gui, preset)
