...
class AlphaBetaPlayer(ComputerPlayer):
    def __init__(
        ...
        if search_depth < 0 and max_time <= 0:
            raise ValueError(
                "AlphaBetaPlayer needs either a search_depth, or a max_time"
            )
    ...
    def move(self, board):
        ...
        if self.search_depth < 0:
            ...
            val, move_list, time_up = self.alpha_beta(
                board,
                self.search_depth,
                -inf,
                inf,
    ...
    def MTD_f(self, board, guess, depth):
        ...
        while lower < upper:
            ...
            transposition_table = dict()
            val, move_list, time_up = self.alpha_beta(
                board, depth, bound - 1, bound, self.player_num, transposition_table
            )
            if val < bound:
                upper = val
            else:
...
class MonteCarloPlayer(ComputerPlayer):
    ...
    def UCB(self, board, state):
        ...
        for next_move in children:
            ...
            board.undo()
            weight = (
                child_state[1] + self.C * (math.log(state[0]) / child_state[0]) ** 0.5
            )
